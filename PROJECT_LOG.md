...
| D49 | **A map pin is identified by coordinate AND price, and an unsure match does nothing.** D25 keyed markers by `position` with a 1e-4 tolerance. But Airbnb reports plenty of coordinates *rounded to 4dp*, so that tolerance is the grid spacing of the data, not a margin on it: measured on one real captured search (`scripts/test-marker-id.js`), 26 listings contain 7 pairs inside 1e-4 and three sit on the identical coordinate. Archiving one flat therefore hid every other flat in the building: silently, on the map only (the panel still listed them), and permanently, because `hideArchivedMarkers` re-applied on every pass. `resolveMarkers()` now returns `{id, cat, confident}` per marker, either unambiguous (one tagged listing, one pin) or disambiguated by the price on the pill, else not confident. **Hiding requires `confident`; colouring and hover take the best guess.** Prices separate 6 of the 7 colliding pairs; the 7th is two identically-priced flats, where the answer is correctly "touch neither". | Asymmetric harm: a lingering archived pin is visible, and the interceptor drops it by id on the next fetch, whereas a listing you never archived vanishing is silent and permanent. Bias every uncertain call towards the recoverable failure. |
| D50 | **Hiding a pin is now half of a pair: `syncArchivedMarkers` also puts pins back.** The old code only ever set `display:none`, so unarchiving (or switching "show archived" on) left the pin gone until Google Maps happened to rebuild that marker, and both controls read as broken. `data-archiver-hidden` stores *which listing* a pin was hidden for, so the restore is exact and never has to re-resolve an ambiguous marker; `pendingArchive` holds it down through the undo window without the restore pass fighting it. | A toggle that only works one way isn't a toggle. Recording the id makes putting it back exact rather than another guess. |
| D51 | **One archive path, `archiveWithUndo`, for every trash button.** D13/D14 gave the map's trash an optimistic vanish plus a 1.5s undo; the panel row's 🗑 committed instantly with no way back, recoverable only from the popup's Archived tab. Since D30 the panel is the surface you actually click, so that was the easiest way to lose a listing by accident. Both callers now pass their own hide/restore into one function. | The inconsistency was invisible until you hit it, and it was on the more-used surface. One function, so they can't drift apart again. |
| D52 | **An empty price probe is retried in the background, not written down as "Unavailable".** `probePrice` treated "no listings in the box" as proof Airbnb won't quote the listing. But a login wall, a rate-limit interstitial and a markup change look identical to it and all answer HTTP 200, and we fire up to 8 probes per render, so the panel could print a confident "Unavailable" over the one number you're deciding on. It now returns `ok` / `empty` / `error`; anything but `ok` retries at 30s, 2min, 10min, and only a listing that has said the same thing every time is called unavailable. Neighbours coming back while this one doesn't is still believed immediately, because that is a real answer. A 60s sweep also re-probes prices past their 15-minute TTL without waiting for a render. `PROBE_RETRY_MS` is read at use time so a test can reach the far end of the backoff. | Wrong information stated confidently is worse than no information. The retry costs nothing; the false "Unavailable" cost a listing. |
| D53 | **Storage updates are never dropped, and writes are serialised.** The background's `refresh()` returned early whenever a load was in flight, and because storage operations are serialised, that in-flight read had already been answered with the pre-change state, so the update was lost until an unrelated write happened along. That is "I archived it and it came back", the failure this whole extension exists to prevent (its guard comment was also wrong: `refresh` writes nothing). It now loops again instead. Separately, every `Store` mutator is a read-modify-write over a shared key, so two in flight at once lost one; they now run through a promise chain. | D3: the archived set is the source of truth. A source of truth that silently misses updates is worse than no cache. |
| D54 | **Two unbounded things bounded.** `seen` in the persistent background page kept every listing of every search in every city, full GraphQL objects, for as long as the browser stayed open. It is a warm cache (so a listing tagged *now* already has photos and a price), so it now keeps the most recent 500 plus anything tagged (`Filter.collectSeen` returns the ids it touched). And `decorateBridge` serialised `document.body.innerHTML` and regexed it on *every* decorate pass: a multi-megabyte string, on a live chat, the page that never stops mutating. The thread's own `/rooms` links carry the same answer and can't change while you stay on the thread, so it reads them from the DOM and caches per pathname, with the full-text scan kept as a 3-try fallback. | Neither had a bound and neither needed one. Both were paid continuously, on the pages used most. |
| D55 | **The panel is pinned to the viewport and the LIST scrolls, with a scrollbar the panel draws itself.** In flow the panel grew to the height of the whole list and let the page scroll, exactly as Airbnb's cards did (25 rows measured 9,941px tall). Reaching the bottom of your own list therefore scrolled the map off screen. The panel is now `position: sticky` with a measured `top` (the same `chromeBottom()` the overlay fallback uses) and a `max-height` of the remaining viewport, so the list scrolls inside it and the map never moves (verified: map top 176px before and after scrolling the list). The bar is **drawn, not native**: on Firefox 153/Windows every combination of `scrollbar-width`, `scrollbar-color` and `scrollbar-gutter` yields a 0px gutter (measured, all 8 combinations), i.e. an overlay bar that reserves no width and fades out. `.archiver-sbar` is an 8px track with a draggable thumb and click-to-jump, hidden when the list doesn't overflow. | User asked for a scroll bar for the list. The platform will not provide a visible one here, and "the list scrolls" is worth nothing if you can't see how far down you are. |
| D56 | **Rows collapse to a strip, in place, and it persists.** A per-row `▾`/`▸` button plus one header `Collapse` / `Expand` for everything on screen. Collapsed keeps the photo (shrunk to 56px), the price and the buttons; it drops the carousel nav, the per-night line, the sub-line, the host row and the note/chat tabs: 380px down to 58px per row, 9,444px down to 1,716px for a list of 25. It deliberately keeps the **drag handle**, so tidying the list never costs you the ability to reorder it, and it changes only height, so every row keeps its slot. Stored under its own `collapsed` key, independent of category like notes and order, so it survives star <-> maybe and a reload. | User asked for a button to collapse list items so they appear smaller but keep their position. Persisting it is the difference between a view control and a toy. |
| D57 | **The pin highlight was real but invisible, and hover now resolves fresh.** FEATURES claimed hovering a row lights up its pin and the user saw nothing happen. It *was* firing: measured on the live page, the class landed and the pill got its outline. The problem was that it amounted to a 1.18x scale and a thin `#222` outline on a 48x28px pill in a busy map. It is now 1.3x, a `#e0115f` ring and a matching glow, every property `!important` because the pill carries inline styles from Airbnb (48x28 to 63x36, verified). **Measuring artefact worth remembering:** that pill has a 200ms `transform` transition and a running transition outranks even `!important`, so reading `getComputedStyle` in the same tick as the class shows the *old* value and looks like the rule was ignored. Always measure it after the transition settles. Hover also resolves markers fresh rather than from the 500ms memo, since Google rebuilds markers as you pan and a cached answer can name elements that have left the document. | "It doesn't happen" turned out to mean "I can't see it". The fix is contrast, not plumbing, and the test now asserts the pin's real box grows rather than just that a class landed. |
