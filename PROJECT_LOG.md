...
| D28 (planned) | **Always-show starred listings** even when Airbnb drops them on map move: interceptor caches each starred listing's full result objects (refreshed whenever seen) and re-injects them into responses when missing but within the returned-results bounds (coords are in the data). Stale info refreshes on next sighting / when the listing's real page is opened. | User: Airbnb prefilters the map and hides some listings; starred ones should always appear. Caveats: may show currently-unavailable listings; only works for listings cached at least once. NOT yet built. |
